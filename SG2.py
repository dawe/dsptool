    ...
    if MergedList:
        for i in MergedList:
            if sg_step > i-j:
                ...
                j = i
                sg_output.addEntries(chr, k, values=[intensity], span=sg_step, step=sg_step)
#----------------------------------------------
...
"""))
parser.add_argument("-i", "--input", help="Define the name and location of the BigWig file as input. e.g. -i path/file.bw ",required=True)
parser.add_argument("-o", "--output", help="Define the name and the location of the output BigWig file. If no output file defined, the result will be stored in Peak.bw file in the running folder.", default="Peaks.bw")
parser.add_argument("-S", "--step", type=int, help="The step size that denoising proccess applied by, the default step size is 50bps.", default=50)
